...
class WebAppTests(unittest.TestCase):
    ...
    def test_sakura_without_assistant_marks_degraded(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            sample_path = root / "demo.epub"
            build_sample_epub(sample_path)
            ...
            with patch("translator.pipeline._build_llm_client", side_effect=lambda *_: FakeClient()):
                client.post(
                    "/start",
                    data={
                        "existing_file": str(sample_path),
                ...
                snapshot = client.get("/api/status").get_json()
                job = snapshot["job"]
                self.assertTrue(job["assistant_degraded_summary"])
                self.assertTrue(job["assistant_degraded_reference"])
                self._wait_for_job_completion(client)
